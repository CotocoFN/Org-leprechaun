...
.data
    node_size:      .word 8  # Tamanho do n� (8 bytes)
    head:           .word 0  # Ponteiro do in�cio da lista
...
# reserva endere�o em ra para ret
    addi sp, sp, -4
    sw ra, 0(sp)
    call insert_element
    lw ra, 0(sp)
...
# confere sucesso ou erro da opera��o
    li t1, 1
...
insert_loop:
    lw t4, 4(t1)         # pr�ximo n�
    beqz t4, insert_end  # achou �ltimo
    mv t1, t4
    j insert_loop

insert_end:
    sw a0, 4(t1)  # �ltimo aponta para o novo n�
    j insert_done
...
# reserva endere�o em ra pra ret
    addi sp, sp, -4
    sw ra, 0(sp)
...
remove_element_by_index:
    ...
    mv a1, a0  # move �ndice para a1
    la a0, head
    call remove

# recupera endere�o de ret
    lw ra, 0(sp)
    addi sp, sp, 4
...
remove:
    ...
    mv t4, t0  # t4 = n� anterior
    mv t2, a1  # t2 = �ndice alvo
    li a6, 1   # a6 = �ndice atual
...
# ---------------------Remove N�--------------------#
remove_element:
    lw t5, 4(t1)    # pega pr�ximo n�
    beq t4, t0, new_head
    sw t5, 4(t4)    # anterior aponta para pr�ximo
    sw zero, 0(t1)  # limpa valor removido
    j remove_done

new_head:
    sw t5, 0(t4)  # head aponta pra pr�ximo
